...
impl RmsNormGated {
    ...
    pub fn forward(&self, x: &Tensor, gate: &Tensor) -> Result<Tensor> {
        ...
        if x.device().is_metal() {
            return crate::metal::gdn::rms_norm_gated_metal(x, gate, &self.weight, self.eps);
        }
    ...
    pub fn forward(&self, x: &Tensor, cache: &mut GdnLayerCache) -> Result<Tensor> {
        ...
        let (q, k) = {
            ...
            if q.device().is_metal() {
                let q = crate::metal::gdn::l2_norm_metal(&q, 1e-6)?;
                let k = crate::metal::gdn::l2_norm_metal(&k, 1e-6)?;
                (q, k)
            } else {
